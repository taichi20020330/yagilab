...
def train_random_forest(X_train, Y_train):
    ...
    skf = StratifiedKFold(n_splits = 5) #クロスバリデーションのfold設定
    gs = GridSearchCV(estimator = clf, param_grid = search_params, cv = skf, n_jobs = -1)
    gs.fit(X_train, Y_train)
    return  gs.best_estimator_

def train_light_gbm(X_train, Y_train):
    search_params = {}
...
def train_knn(X_train, Y_train):
    #クロスバリデーションを用いたグリッドサーチ
    search_params = {'n_neighbors':range(1, 20)}
...
def train_native_bayes(X_train, Y_train):
    clf = GaussianNB()
    clf.fit(X_train, Y_train)
    return clf
...
def main():
    ...
    for k in range(10):
        ...
        # SVMモデルの学習
        clf = train_random_forest(X_train_pca, Y_train)
